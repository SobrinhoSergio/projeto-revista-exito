...
<div class="users-list-table">
    ...
    <?= $type_sponsor_name ?>
    <?php if ($type_sponsor_name == "") { echo "Listando todos os anúncios";} ?>
    ...
    <table>
        ...
        <?php while($result = $queryPaginated->fetch_assoc()): ?>
            ...
            <tr>
                ...
                <td>
                    <div class="td-div">
                        <a href="sponsor_edit_controller.php?sponsor_id=<?php echo $result['chave_anuncio'];?>">Editar</a>
                    </div>
                </td>
            </tr>
        <?php endwhile ?>
    ...
    <div class="list-pages">
        ...
        if (@$_GET['page'] != 1 && isset($_GET['page'])) {
            echo "<a href='sponsors_list_controller.php?page=$previousPage"; if(isset($_GET['type'])) { echo "&type=".$_GET['type']; } echo "' class='list-arrow-anchor'> &#x2039; </a>";
        }
        if ($_GET['page'] <=3) {

            echo "<a href='sponsors_list_controller.php?page=1"; if(isset($_GET['type'])) { echo "&type=".$_GET['type']; } echo "' class='list-page-anchor ";
            if ($_GET['page'] == 1) {echo 'list-current-page';}
            echo "'> 1</a>";

            if ($pages >= 2) {
                echo "<a href='sponsors_list_controller.php?page=2";if(isset($_GET['type'])) { echo "&type=".$_GET['type']; } echo "' class='list-page-anchor ";
                if ($_GET['page'] == 2) {echo 'list-current-page';}
                echo "'> 2</a>";
            }
            if ($pages >= 3) {
                echo "<a href='sponsors_list_controller.php?page=3";if(isset($_GET['type'])) { echo "&type=".$_GET['type']; } echo "' class='list-page-anchor ";
                if ($_GET['page'] == 3) {echo 'list-current-page';}
                echo "'> 3</a>";
            }
        } else {
            echo "<a href='sponsors_list_controller.php?page=$beforePreviousPage"; if(isset($_GET['type'])) { echo "&type=".$_GET['type']; } echo "' class='list-page-anchor'> $beforePreviousPage </a>";
            echo "<a href='sponsors_list_controller.php?page=$previousPage";if(isset($_GET['type'])) { echo "&type=".$_GET['type']; } echo "' class='list-page-anchor'> $previousPage </a>";
            echo "<a href='sponsors_list_controller.php?page=$currentPage";if(isset($_GET['type'])) { echo "&type=".$_GET['type']; } echo "' class='list-page-anchor list-current-page'> $currentPage </a>";
        }

        // ao chegar na última página, não mostrar o botão 'próximo'
        if (@$_GET['page'] < $pages) {
            if (@$_GET['page'] >= 3) {
                echo "<a href='sponsors_list_controller.php?page=$nextPage";if(isset($_GET['type'])) { echo "&type=".$_GET['type']; } echo "' class='list-page-anchor'> $nextPage </a>";
            }
            echo "<a href='sponsors_list_controller.php?page=$nextPage";if(isset($_GET['type'])) { echo "&type=".$_GET['type']; } echo "' class='list-arrow-anchor'> &#x203A; </a>";
        }
