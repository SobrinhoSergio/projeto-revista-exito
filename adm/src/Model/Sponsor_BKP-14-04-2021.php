...
class Sponsor extends Model implements Image
{
    ...
    public function insertSponsor()
    {
        ...
        $sql = "INSERT INTO tabela_anuncios (descricao_anuncio, tipo_anuncio, publicar, link_anuncio, vencimento_anuncio, insercao_anuncio) VALUES (?,?,?,?,?,?)";

        $stmt = $cnn->prepare($sql);
        $result = $stmt->bind_param("siisss", $this->description, $this->type, $publish, $this->link, $exp_date, $currentDate);
    ...
    public function update($key)
    {
        ...
        if ($this->expiration != 0) {
            Database::getResultFromQuery("UPDATE tabela_anuncios SET vencimento_anuncio = DATE_ADD(CURRENT_DATE(), INTERVAL (".$this->expiration.") MONTH) WHERE chave_anuncio = ". $key);
        }
    ...
    public function getRandom($limit, $filters = [])
    {
        ...
        $sql.= " ORDER BY chave_anuncio DESC LIMIT " . $limit;
   ...
   public function resize_image($file, $newName, $w, $h)
    ...
    public static function republishSlideshow($link) {
        ...
        $sql = "UPDATE tabela_anuncios SET ativo = 1, publicar = 1, vencimento_anuncio = (?) WHERE link_anuncio = (?)";
