...
class Post extends Model implements Image
{
    ...
    {
        $cnn = Database::getConnection();
        $sql = "SELECT * FROM tabela_materias WHERE ativo = 1";
    ...
    public static function searchPosts($filter)
    {
        ...
        LOWER(tipo_materia) LIKE LOWER('%" . $filterLower . "%') OR
        UPPER(tipo_materia) LIKE UPPER('%" . $filterUpper . "%') OR
    ...
    public static function searchPostsTipo($filter)
    {

        $sql = "SELECT * FROM tabela_materias WHERE tipo_materia = $filter AND ativo = 1 AND publicar = 1 ORDER BY chave_materia DESC";
    ...
    {
        $sql = "SELECT * FROM tabela_materias WHERE publicar = 1 AND ativo = 1";
        if ($filters != []) {
            $sql .= " AND 1=1";
            foreach ($filters as $index => $value) {
                $sql .= " AND $index = $value";
