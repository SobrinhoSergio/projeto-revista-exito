...
<div class="container">
    <div class="content">
        ...
        <div class="videos-content">
            ...
            <?php while ($relatedVideos = $videos->fetch_assoc()): ?>

                <?php if ($relatedVideos['chave_video'] != $fields['chave_video']): ?>
                    <?php
                    // pega id do vídeo para usar na thumbnail
                    $match = preg_match('/^.*(youtu\.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*).*/',
                        $relatedVideos['link_video'], $matches, PREG_OFFSET_CAPTURE);
                    $video_id = $matches[2][0];
                    ?>

                    <?php if ($i == 0): ?>
                    <!-- card da esquerda -->
                    <div class="video-card video-relacionado mobile-padding-right"
                         onclick="window.location.href='video.php?video_id=<?=$relatedVideos['chave_video']?>'" style="cursor:pointer;">
                        <?php else: ?>
                        <!-- card da direita -->
                        <div class="video-card video-relacionado mobile-padding-left"
                             onclick="window.location.href='video.php?video_id=<?=$relatedVideos['chave_video']?>'" style="cursor:pointer;">
                            <?php endif ?>

                            <div class="video-card-subject titulo-padrao"><h3><?=$relatedVideos['tema_video']?></h3></div>

                            <a class="video-card-img" href="video.php?video_id=<?=$relatedVideos['chave_video']?>" style="text-decoration: none; color: inherit;">
                            <img style="width: 100%;" src="https://img.youtube.com/vi/<?=getVideoId($relatedVideos['link_video'])?>/sddefault.jpg" alt="<?=$relatedVideos['titulo_video']?>">
                            </a>
        ...
        <!--                            --><?//= substr(strip_tags($relatedVideos['descricao_video']), 0, 190)?><!-- (...)-->
